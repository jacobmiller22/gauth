...
func (app *gauthApp) authorize(w http.ResponseWriter, r *http.Request) {
...
func (app *gauthApp) token(w http.ResponseWriter, r *http.Request) {
	...
	queryParams := r.URL.Query()
	// clientId := queryParams.Get("client_id")         // REQUIRED IF NO OTHER AUTH PRESENT
	// clientSecret := queryParams.Get("client_secret") // POTENTIALLY REQUIRED
	// code := queryParams.Get("code")                  // REQUIRED
	// grantType := queryParams.Get("grant_type")       // REQUIRED
	// codeVerifier := queryParams.Get("code_verifier") // PKCE ONLY
	// redirectUri := queryParams.Get("redirect_uri")   // POTENTIALLY REQUIRED
	log.Printf("Token Handler called w/ Query Params: %v", queryParams)
